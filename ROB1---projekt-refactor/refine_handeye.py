...
if __name__ == "__main__":
    cam2base = np.load("./calibration_data/handeye_output_streda/cam2base.npy")
    target2gripper = np.load(
        "./calibration_data/handeye_output_streda/target2gripper.npy"
    )
    K = np.load("./calibration_data/cam_params/K.npy")
    dist = np.load("./calibration_data/cam_params/dist.npy")
    target_method = CharucoPointMatcher((5, 5), 0.02, 0.015)
    data_dir = Path("datasets/handeye_data/")
    out_dir = Path("calibration_data/handeye_output_streda_refined/")
