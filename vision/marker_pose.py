...
class MarkerPoseSolver(Protocol):
    def __call__(
        self,
        img: np.ndarray,
...
class CharucoPoseSolver(MarkerPoseSolver):
    def __init__(
        self,
        K: np.ndarray,
        dist: np.ndarray,
...
class ArucoPoseSolver(MarkerPoseSolver):
    def __init__(
        self,
...
if __name__ == "__main__":
    ...
    method = CharucoPoseSolver(K, dist, (4, 4), 25, 18)
    x = get_marker_pose(img, method, vis=True)
    print(x)
